...
@app.route("/test")
def index():
    msg = Message(subject='Hello from the other side!', sender='devhadvani147@gmail.com', recipients=['devhadvani123@gmail.com'])
...
def login():
    if request.method == "POST":
        ...
        if user and check_password_hash(user.password, password):
            token = jwt.encode({'public_id': user.email, 'exp': datetime.utcnow() + timedelta(days=1)}, app.config['SECRET_KEY'], algorithm='HS256')
...
def shop_products():
    ...
    if name:
        products = Product.query.filter(Product.name.like(f"%{name}%")).paginate(page=page, per_page=3)
    else:
...
def send_order_confirmation_email(recipient_email, cart_items):
    msg = Message(subject='Order Confirmation', sender='devhadvani147@gmail.com', recipients=[recipient_email])
...
def check_out():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            print("Failed to send order confirmation email. Please contact support.", "error")
        db.session.delete(cart)
...
@token_require
def my_orders():
    order = Order.query.filter_by(user_id = g.current_user.id).order_by(desc(Order.order_date)).all()
...
def account():
    ...
    order = Order.query.filter_by(user_id = user.id).order_by(desc(Order.order_date)).all()
